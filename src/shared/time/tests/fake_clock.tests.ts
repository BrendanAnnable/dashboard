...
describe('FakeClock', () => {
    ...
    it('starts at unix epoch by default', () => {
      expect(
        FakeClock.of()
          .date()
          .toUTCString(),
      ).toEqual('Thu, 01 Jan 1970 00:00:00 GMT');
    });

    it('returns date at the initialized time', () => {
      expect(
        FakeClock.of(1492778724)
          .date()
          .toUTCString(),
      ).toEqual('Fri, 21 Apr 2017 12:45:24 GMT');
    });
  });
  ...
  describe('#runOnlyPendingTimers', () => {
    ...
    it('throws an exception where there are too many scheduled tasks', () => {
      ...
      expect(() => clock.runOnlyPendingTimers()).toThrow(
        /Exceeded clock task limit/,
      );
    });
  });
  ...
  describe('#runTimersToTime', () => {
    ...
    it('throws an exception where there are too many scheduled tasks', () => {
      ...
      expect(() => clock.runTimersToTime(2000)).toThrow(
        /Exceeded clock task limit/,
      );
    });
  });
