...
import { Simulator, NetworkSend } from './simulator';

import { periodic } from './periodic';
// import Sensors = message.input.Sensors;

export class DashboardSimulator extends Simulator {
  constructor(
  ...
  get sensors(): Message {
    ...
    const angle =
      (this.robotIndex * (2 * Math.PI)) / this.numRobots + time / 40;
...
function toHtw(x: number, y: number, z: number, heading: number): Matrix4 {
  ...
  return new Matrix4().getInverse(
    new Matrix4().compose(translation, rotation, scale),
  );
}
